...
def opticalFlowTrack(img0, img1, pts0, back_check, horizontal_check):
    ...
    if back_check is True:
        reverse_pts0, reverse_status, err = \
            cv2.calcOpticalFlowPyrLK(
                img1, img0, pts1, (21, 21), 1, criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 30, 0.01))
    ...
    def draw(self, img, status):
        ...
        for i in range(self.pts1.shape[0]):
            ...
            cv2.arrowedLine(image_show,
                            tuple(self.pts1[i][0].astype(int)),
                            tuple(self.pts0[i][0].astype(int)), (0, 255, 0), 1, 8, 0, 0.2)
