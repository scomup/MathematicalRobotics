...
class PreintegrationExample:
    ...
    def run(self, T: int = 12):
        ...
        for i, t in enumerate(np.arange(0, T, self.dt)):
            ...
            if i % 25 == 0:
                pose.append(np.concatenate([np.array([t]),cur_pose.translation(), cur_pose.rotation().quaternion()]))
        ...
        np.save('/home/liu/bag/pose0.npy', np.array(pose))
...
if __name__ == '__main__':
    PreintegrationExample(bias=gtsam.imuBias.ConstantBias(np.array([0.01, 0.02, 0.05]), np.array([0.01, 0.02, 0.05]))).run()
