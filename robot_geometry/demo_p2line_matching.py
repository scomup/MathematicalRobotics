...
class p2lineEdge:
    def __init__(self, i, z, omega = None, color = 'black'):
        self.i = i
        self.z = z
        self.type = 'one'
        self.color = color
...
if __name__ == '__main__':
    ...
    #tar = np.array([[1.5,1.5,1.2],[0.4,0.7,0.5],[2,2.2,2]])
    tar = np.array([[1.5,1.5,1.5],[0.5,0.5,0.5],[2,2.2,2]])
    ...
    for p in tar:
        graph.addEdge(p2lineEdge(0,[p, center, direction])) # add prior pose to graph
        r, j = point2line(p, center, direction, True)
        draw_arrow(ax, p, j)

    ax.scatter(ref[:,0],ref[:,1],ref[:,2],label='ref')
    ax.scatter(tar[:,0],tar[:,1],tar[:,2],label='tar')
    graph.solve(min_score_change = 0.00001, step = 0.05)
    x = graph.nodes[0].x
