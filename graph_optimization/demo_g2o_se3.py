...
def add(x1, x2):
    if len(x2) == 7:
        return np.array([x1[0] + x2[0] + 2. * (-(x1[4]**2 + x1[5]**2) * x2[0] + (x1[3] * x1[4] - x1[5] * x1[6]) * x2[1] + (x1[3] * x1[5] + x1[4] * x1[6]) * x2[2]),
                ...
                x1[6] * x2[3] + x1[3] * x2[6] + x1[4] * x2[5] - x1[5] * x2[4],
                x1[6] * x2[4] - x1[3] * x2[5] + x1[4] * x2[6] + x1[5] * x2[3],
                x1[6] * x2[5] + x1[3] * x2[4] - x1[4] * x2[3] + x1[5] * x2[6],
                x1[6] * x2[6] - x1[3] * x2[3] - x1[4] * x2[4] - x1[5] * x2[5]])
    if len(x2) == 6:
...
class Pose3dbetweenEdge:
    ...
    def residual(self, vertices):
        v1 = vertices[self.i].x
        v2 = vertices[self.j].x

        r = sub(sub(v2, v1), self.z)[:6]
        J1 = J_x1_minus_x2_dx1(sub(v2, v1), self.z)[:6] @ J_x1_minus_x2_dx2(v2, v1) @  J_plus(v1)
        J2 = J_x1_minus_x2_dx1(sub(v2, v1), self.z)[:6] @ J_x1_minus_x2_dx1(v2, v1) @  J_plus(v2)
