...
class ParticleFilter:
    def __init__(self, num_particles, initial_state, motion_model, measurement_model):
        ...
        self.particles = np.array([initial_state.to_array() for _ in range(num_particles)])
    ...
    def predict(self, u, dt):
        for i in range(self.num_particles):
            state = State2D(self.particles[i][:2], expSO2(self.particles[i][2]), self.particles[i][3:])
            predicted_state, _, _ = self.motion_model.predict(state, u, dt, noise=True)
    ...
    def correct(self, z):
        for i in range(self.num_particles):
            ...
            self.weights[i] = self.gaussian_likelihood(z, predicted_measurement, self.measurement_model.R)
