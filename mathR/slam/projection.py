...
if __name__ == '__main__':
    ...
    r, J = pose_inv(x, True)
    Jm = numerical_derivative(pose_inv, [x], 0, pose_plus, pose_minus)
    print('%s test pose_inv' % check(J, Jm))
    ...
    print('%s test pose_plus' % check(x3m, x3))
    print('%s test pose_minus' % check(x2m, x2))

    J1m = numerical_derivative(pose_plus, [x1, x2], 0, pose_plus, pose_minus)
    J2m = numerical_derivative(pose_plus, [x1, x2], 1, pose_plus, pose_minus)
    print('%s test pose_plus J1' % check(J1m, J1))
    print('%s test pose_plus J2' % check(J2m, J2))

    r, J1, J2 = pose_minus(x1, x2, True)
    J1m = numerical_derivative(pose_minus, [x1, x2], 0, pose_plus, pose_minus)
    J2m = numerical_derivative(pose_minus, [x1, x2], 1, pose_plus, pose_minus)

    print('%s test pose_minus J1' % check(J1m, J1))
    print('%s test pose_minus J2' % check(J2m, J2))
    x = np.array([0.1, 0.3, 0.5, 0.1, 0.2, 0.3])
    p = np.array([5., 6., 10.])
    r, J1, J2 = transform(x, p, True)
    J1m = numerical_derivative(transform, [x, p], 0, pose_plus)
    J2m = numerical_derivative(transform, [x, p], 1)
    print('%s test transform J1' % check(J1m, J1))
    print('%s test transform J2' % check(J2m, J2))

    r, J1, J2 = transform_inv(x, p, True)
    J1m = numerical_derivative(transform_inv, [x, p], 0, pose_plus)
    J2m = numerical_derivative(transform_inv, [x, p], 1)
    print('%s test transform_inv J1' % check(J1m, J1))
    print('%s test transform_inv J2' % check(J2m, J2))

    r, J = reproject(p, K, True)
    Jm = numerical_derivative(reproject, [p, K], 0)
    print('%s test project J1' % check(J, Jm))
    ...
    J1m = numerical_derivative(reproj_error, [x, p, pim, K], 0, pose_plus, delta=1e-8)
    J2m = numerical_derivative(reproj_error, [x, p, pim, K], 1)
    print('%s test reproj_error J1' % check(J1m, J1))
    print('%s test reproj_error J2' % check(J2m, J2))
    ...
    J1m = numerical_derivative(reproj_error0, [x, p, pim, K], 0, pose_plus, delta=1e-8)
    J2m = numerical_derivative(reproj_error0, [x, p, pim, K], 1)
    print('%s test project_error0 J1' % check(J1m, J1))
    print('%s test project_error0 J2' % check(J2m, J2))
